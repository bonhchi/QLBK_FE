...
@Component({
  ...
  styleUrls: ['./report05.component.scss']
})
export class Report05Component implements OnInit {
  ...
  constructor(
    ...
    this.isQCRole = this.currentUser.role.some(e => e == 'QC');
  ...
  private async _getReportList() {
    const report_id = '05_KIEM_KE_QUY_CUOI_NGAY_DVTQ'; // Sổ kiểm kê quỹ giữa giờ
    try {
      this.isLoading = true;
      this._spinner.show();
  ...
  public getFormRequest() {
    const request = {
      report_id: '05_KIEM_KE_QUY_CUOI_NGAY_DVTQ',
  ...
  public openModal(type: string, code: any, item: any) {
    if (type == 'Add') {
      this.dialogCheckRef = this.dialog.open(ReportCreateComponent, {
        width: '70%',
        data: {
          report_type: '05_KIEM_KE_QUY_CUOI_NGAY_DVTQ',
