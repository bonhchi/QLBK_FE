...
@Component({
  ...
  styleUrls: ['./report03.component.scss']
})
export class Report03Component implements OnInit {
  ...
  private async _getReportList() {
    const report_id = '03_KIEM_KE_QUY_CUOI_NGAY_SAU_HQ'; // Sổ kiểm kê quỹ giữa giờ
    try {
      this.isLoading = true;
      this._spinner.show();
  ...
  public getFormRequest() {
    const request = {
      report_id: '03_KIEM_KE_QUY_CUOI_NGAY_SAU_HQ',
      tran_date: moment(this.f.report_date.value, 'DD/MM/YYYY').format('YYYY-MM-DD') || null,
  ...
  public openModal(type: string, code: any, item: any) {
    if (type == 'Add') {
      this.dialogCheckRef = this.dialog.open(ReportCreateComponent, {
        width: '70%',
        data: {
          report_type: '03_KIEM_KE_QUY_CUOI_NGAY_SAU_HQ',
