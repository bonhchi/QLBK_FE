...
export class Report10Component implements OnInit {
  ...
  private async _getReportList() {
    const report_id = '10_BB_KIEM_KE_BAN_GIAO_TAI_SAN_DVKTQ'; // Sổ kiểm kê quỹ giữa giờ
    try {
      this.isLoading = true;
      this._spinner.show();
  ...
  public getFormRequest() {
    const request = {
      report_id: '10_BB_KIEM_KE_BAN_GIAO_TAI_SAN_DVKTQ',
      tran_date: '',
  ...
  public openModal(type: string, code: any, item: any) {
    if (type == 'Add') {
      this.dialogCheckRef = this.dialog.open(ReportCreateComponent, {
        width: '70%',
        data: {
          report_type: '10_BB_KIEM_KE_BAN_GIAO_TAI_SAN_DVKTQ',
