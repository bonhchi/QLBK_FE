...
import { ReportUpdateComponent } from '../modal/report-update/report-update.component';
...
export class Report01Component implements OnInit {
  ...
  constructor(
    ...
    this.isRole = this.currentUser.role.some(e =>  {return e == 'QP' || e == 'GDV'});
  ...
  private _formControl() {
    this.form = this._formBuilder.group({
      ...
      report_user: [{ value: this.isDisableParentBranch ? '' : this.currentUser.username.toUpperCase(), disabled: !this.isDisableParentBranch }],
    });
  }

  // Get API
  private async _getReportList() {
    const report_id = '01_KIEM_KE_QUY_GIUA_GIO_DOT_XUAT_THQ';
    try {
      this.isLoading = true;
      this._spinner.show();
  ...
  public getFormRequest() {
    const request = {
      report_id: '01_KIEM_KE_QUY_GIUA_GIO_DOT_XUAT_THQ',
      tran_date: '',
      branch_id: this.f.report_branch.value,
      user_id: this.isDisableParentBranch ? '' : this.currentUser.username.toUpperCase(),
      // user_id: '',
      // tran_type: '',
  ...
  public openModal(type: string, code: any, item: any) {
    if (type == 'Add') {
      this.dialogCheckRef = this.dialog.open(ReportCreateComponent, {
        width: '70%',
        data: {
          report_type: '01_KIEM_KE_QUY_GIUA_GIO_DOT_XUAT_THQ',
      ...
      this.dialogCheckRef = this.dialog.open(ReportUpdateComponent, {
        width: '70%',
        data: {
          item,
          type: this.reportType,
