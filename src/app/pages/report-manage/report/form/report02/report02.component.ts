...
@Component({
  ...
  styleUrls: ['./report02.component.scss']
})
export class Report02Component implements OnInit {
  ...
  private async _getReportList() {
    const report_id = '02_KIEM_KE_QUY_CUOI_NGAY_TRUOC_HQ';
    try {
      this.isLoading = true;
      this._spinner.show();
  ...
  public getFormRequest() {
    const request = {
      report_id: '02_KIEM_KE_QUY_CUOI_NGAY_TRUOC_HQ',
      tran_date: '',
  ...
  public openModal(type: string, code: any, item: any) {
    if (type === 'Add') {
      this.dialogCheckRef = this.dialog.open(ReportCreateComponent, {
        width: '70%',
        data: {
          report_type: '02_KIEM_KE_QUY_CUOI_NGAY_TRUOC_HQ',
