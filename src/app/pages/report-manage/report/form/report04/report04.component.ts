...
/*  */@Component({
  ...
  styleUrls: ['./report04.component.scss']
})
export class Report04Component implements OnInit {
  ...
  private async _getReportList() {
    const report_id = '04_KIEM_KE_QUY_GIUA_GIO_DOT_XUAT_DV';
    try {
      this.isLoading = true;
      this._spinner.show();
  ...
  public getFormRequest() {
    const request = {
      report_id: '04_KIEM_KE_QUY_GIUA_GIO_DOT_XUAT_DV',
  ...
  public openModal(type: string, code: any, item: any) {
    if (type == 'Add') {
      this.dialogCheckRef = this.dialog.open(ReportCreateComponent, {
        width: '70%',
        data: {
          report_type: '04_KIEM_KE_QUY_GIUA_GIO_DOT_XUAT_DV',
