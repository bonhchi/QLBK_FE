...
@Component({
  ...
  styleUrls: ['./report09.component.scss']
})
export class Report09Component implements OnInit {
  ...
  private async _getReportList() {
    const report_id = '09_BB_KIEM_KE_BAN_GIAO_TAI_SAN_DVTQ'; // Sổ kiểm kê quỹ giữa giờ
    try {
      this.isLoading = true;
      this._spinner.show();
  ...
  public getFormRequest() {
    const request = {
      report_id: '09_BB_KIEM_KE_BAN_GIAO_TAI_SAN_DVTQ',
      tran_date: '',
  ...
  public openModal(type: string, code: any, item: any) {
    if (type == 'Add') {
      this.dialogCheckRef = this.dialog.open(ReportCreateComponent, {
        width: '70%',
        data: {
          report_type: '09_BB_KIEM_KE_BAN_GIAO_TAI_SAN_DVTQ',
