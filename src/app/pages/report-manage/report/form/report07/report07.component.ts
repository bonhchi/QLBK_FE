...
@Component({
  ...
  styleUrls: ['./report07.component.scss']
})
export class Report07Component implements OnInit {
  ...
  private async _getReportList() {
    const report_id = '07_BB_KIEM_KE_QUY_TAI_SAN_KHAC';
    try {
      this.isLoading = true;
      this._spinner.show();
  ...
  public getFormRequest() {
    const request = {
      report_id: '07_BB_KIEM_KE_QUY_TAI_SAN_KHAC',
      tran_date: '',
  ...
  public openModal(type: string, code: any, item: any) {
    if (type == 'Add') {
      this.dialogCheckRef = this.dialog.open(ReportCreateComponent, {
        width: '70%',
        data: {
          report_type: '07_BB_KIEM_KE_QUY_TAI_SAN_KHAC',
