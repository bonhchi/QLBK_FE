...
@Component({
  selector: 'app-update-advance-fund-start-day',
  templateUrl: './update-advance-fund-start-day.component.html',
  styleUrls: ['./update-advance-fund-start-day.component.scss']
})
export class UpdateAdvanceFundStartDayComponent implements OnInit {
  form: FormGroup;
  ...
  constructor(
    public dialogRef: MatDialogRef<UpdateAdvanceFundStartDayComponent>,
  ...
  public async onUpdate() {
    ...
    if (this.form.value.amount == 0) {
      this._notificationService.error('Thông báo',  "Số tiền phải lớn hơn 0, Bạn mới có thể ứng quỹ đầu ngày" );
      return;
    }
    ...
    try {
      ...
      this._notificationService.success('Thành công', 'Cập nhật ứng quỹ đầu ngày' );
    } catch (error: any) {
      this._notificationService.error('Thông báo', (error.error?.message) ? error.error.message : error.message);
    }
