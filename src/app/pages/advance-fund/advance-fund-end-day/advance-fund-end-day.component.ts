...
import { AddAdvanceFundEndDayComponent } from './modal/add-advance-fund-end-day/add-advance-fund-end-day.component';
import { DetailAdvanceFundEndDayComponent } from './modal/detail-advance-fund-end-day/detail-advance-fund-end-day.component';
import { UpdateAdvanceFundEndDayComponent } from './modal/update-advance-fund-end-day/update-advance-fund-end-day.component';
import { UserService } from '@app/_services/user.service';
@Component({
  selector: 'app-advance-fund-end-day',
  templateUrl: './advance-fund-end-day.component.html',
  styleUrls: ['./advance-fund-end-day.component.scss']
})
export class AdvanceFundEndDayComponent implements OnInit {
  ...
  private _formControl() {
    this.form = this._formBuilder.group({
      tran_type: [{ value: 'HOAN_QUY_CUOI_NGAY', disabled: false }],
  ...
  private async _getTranType() {
    try {
      this.transaction_type = (await this._categoryService.getCategoryByCode('HOAN_QUY_CUOI_NGAY')).data;
      this.transaction_type.splice(0, 0, { id: '', code: '', name: 'Tất cả' });
    } catch (error) {
      this._notificationService.error('Thông báo', (error.error?.message) ? error.error.message : error.message);
    }
  }
  ...
  public openForm(type: string, item: any) {
    switch (type) {
      case 'DETAIL':
        this.dialogRef = this.dialog.open(DetailAdvanceFundEndDayComponent, {
          width: '70vw',
          data: { type, item, ccyList: this.ccyList,transaction_type: this.transaction_type }
        });
        ...
        break;
      case 'EDIT':
        this.dialogRef = this.dialog.open(UpdateAdvanceFundEndDayComponent, {
          width: '70vw',
        ...
        break;
      case 'ADD':
        this.dialogRef = this.dialog.open(AddAdvanceFundEndDayComponent, {
          width: '70vw',
