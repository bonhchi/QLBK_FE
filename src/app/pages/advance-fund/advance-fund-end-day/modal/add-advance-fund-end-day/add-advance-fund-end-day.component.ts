...
@Component({
  selector: 'app-add-advance-fund-end-day',
  templateUrl: './add-advance-fund-end-day.component.html',
  styleUrls: ['./add-advance-fund-end-day.component.scss']
})
export class AddAdvanceFundEndDayComponent implements OnInit {
  form: FormGroup;
  ...
  constructor(
    public dialogRef: MatDialogRef<AddAdvanceFundEndDayComponent>,
  ...
  private _formControl() {
    this.form = this._formBuilder.group({
      tran_type: [{ value: 'HOAN_QUY_CUOI_NGAY', disabled: true }],
      ...
      description: [{ value: `${this.currentUser?.username.toUpperCase()} HOAN QUY VE `, disabled: false }],
    })
  }
  ...
  public onOptionsSelected(event) {
    ...
    this.f['description'].setValue(`${this.currentUser?.username.toUpperCase()} HOAN QUY VE ${till_vault_id != null ? till_vault_id.toUpperCase() : ''}`);
  }
  ...
  private async _getUserRole() {
    ...
    try {
      ...
      this.userRoleList = res.filter(item => item.role_fcc != null)
    } catch (error: any) {
  ...
  private async _getManagerTypeCcyList() {
    try {
      const request = {
        ...
        till_vault_id: this.currentUser.username
      };
  ...
  public async onSave() {
    ...
    if (this.form.value.amount == 0) {
      this._notificationService.error('Thông báo', "Số tiền phải lớn hơn 0, Không thể Hoàn quỹ cuối ngày");
      return;
    }
