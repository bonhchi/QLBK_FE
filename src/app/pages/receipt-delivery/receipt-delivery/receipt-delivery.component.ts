...
import { AddReceiptDeliveryComponent } from './modal/add-receipt-delivery/add-receipt-delivery.component';
import { DetailReceiptDeliveryComponent } from './modal/detail-receipt-delivery/detail-receipt-delivery.component';
import _clonceDeep from 'lodash/cloneDeep';

@Component({
  selector: 'app-receipt-delivery',
  templateUrl: './receipt-delivery.component.html',
  styleUrls: ['./receipt-delivery.component.scss'],
})

export class ReceiptDeliveryComponent implements OnInit {
  ...
  openForm(type: string, item: any) {
    if (type === 'ADD') {
      ...
      const dialogRef = this.dialog.open(DetailReceiptDeliveryComponent, {
        width: '85vw',
        data: { item, transTypeList: this.transTypeOrgList, statusList: this.statusList }
      });
      dialogRef.afterClosed().subscribe((result) => {
        if (result === 'detail') {
          this._getList();
